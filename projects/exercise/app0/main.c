...
void setup_other_app()
{
	...
	s3k_cap_derive(RAM_MEM, free_cap_mem_idx,
		       s3k_mk_memory(APP_1_BASE_ADDR,
				     APP_1_BASE_ADDR + APP_1_SIZE,
				     S3K_MEM_RWX));
	uint32_t free_cap_idx = find_free_cap();
	s3k_cap_derive(free_cap_mem_idx, free_cap_idx,
		       s3k_mk_pmp(app1_addr, S3K_MEM_RWX));
	s3k_mon_cap_move(MONITOR, APP0_PID, free_cap_idx, APP1_PID,
			 APP_1_CAP_PMP_MEM);
	s3k_mon_pmp_load(MONITOR, APP1_PID, APP_1_CAP_PMP_MEM,
			 APP_1_PMP_SLOT_MEM);
	...
	free_cap_idx = find_free_cap();
	s3k_cap_derive(UART_MEM, free_cap_idx,
		       s3k_mk_pmp(uart_addr, S3K_MEM_RW));
	s3k_mon_cap_move(MONITOR, APP0_PID, free_cap_idx, APP1_PID,
			 APP_1_CAP_PMP_UART);
	s3k_mon_pmp_load(MONITOR, APP1_PID, APP_1_CAP_PMP_UART,
			 APP_1_PMP_SLOT_UART);
...
int main(void)
{
	...
	while (1) {
		...
		if (fr == FR_OK) {
			...
			f_read(&Fil, buffer, 1024,
			       &bw);	    /*Read data from the file */
			fr = f_close(&Fil); /* Close the file */
			if (fr == FR_OK) {
				alt_puts("0> Booting app1 \n");
				memcpy((void *)APP_1_BASE_ADDR, buffer, 1024);
				s3k_mon_reg_write(
				    MONITOR, APP1_PID, S3K_REG_PC,
				    APP_1_BASE_ADDR); //reset the PC to the initial address
				s3k_mon_resume(MONITOR, APP1_PID);
			} else {
