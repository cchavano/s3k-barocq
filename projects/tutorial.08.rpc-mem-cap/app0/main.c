...
uint32_t setup_buffer_cap()
{
	...
	log_sys("app 0: derive cap for shared buffer",
		s3k_cap_derive(
		    RAM_MEM, new_cap,
		    s3k_mk_memory(SHARED_BUFFER_BASE,
				  SHARED_BUFFER_BASE + SHARED_BUFFER_SIZE,
				  S3K_MEM_RW)));
	return new_cap;
}
...
uint32_t setup_buffer_pmp(uint32_t parent_cap_idx)
{
	...
	log_sys("app 0: derive new mem cap",
		s3k_cap_derive(parent_cap_idx, pmp_cap_idx,
			       s3k_mk_pmp(s3k_napot_encode(SHARED_BUFFER_BASE,
							   SHARED_BUFFER_SIZE),
					  S3K_MEM_RW)));
	log_sys("app 0: load pmp for new buffer",
		s3k_pmp_load(pmp_cap_idx, BUFFER_PMP));
	s3k_sync_mem();
	return pmp_cap_idx;
}
